...
text = """Stars burn a very light type of air that is packed very tight in the
...
def blunt_word(text):
    replacetext = text.replace(",", " ").replace(".", " ").casefold()
    bluntword = replacetext.split()
    Top_10_words = Counter(bluntword).most_common(10)
    return Top_10_words
