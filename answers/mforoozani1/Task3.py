...
def best_run(result):
    ...
    for key, value in result.items():
        ...
        A.sort()
    print(
          "the max log likelihood value is", A[0][1], "for this ExaML run name"
          , A[0][0])


def main():
    A = split_replace_remove_dict(string)
    print_keyvalue(A)
    best_run(A)
