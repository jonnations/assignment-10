...
def ordered(tree_dict):
    # sorts the dictionary by occurances, then prints
    sorted(tree_dict.items(), key=operator.itemgetter(1), reverse=True)
    for keys,values in tree_dict.items():
        print(keys,"        ", values)

def best(tree_dict):
    best_tree = sorted(tree_dict.items(), key=operator.itemgetter(1), reverse=True)[-1:]
    print("the best run is", best_tree)
